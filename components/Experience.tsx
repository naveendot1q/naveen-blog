...
const experiences = [
  ...
  {
    role: 'Cloud & DevOps Projects (Self-initiated)',
    company: 'Independent',
    period: '2022 – 2025',
    location: 'Remote',
    current: false,
  ...
  return (
    <section id="experience" className="py-24 px-6">
      <div className="max-w-6xl mx-auto">
          ...
          <div className="space-y-12">
            {experiences.map((exp, i) => (
              <div key={i} className="relative pl-8 md:pl-20">
                {/* Timeline dot */}
                <div
                  className={`absolute left-[-5px] md:left-[18px] top-1.5 w-3 h-3 rounded-full border-2 ${
                    exp.current
                      ? 'bg-[var(--accent)] border-[var(--accent)] shadow-[0_0_8px_var(--accent)]'
                      : 'bg-[var(--surface)] border-[var(--border)]'
                  }`}
                />

                {/* Card */}
                <div className="p-6 rounded-xl border border-[var(--border)] bg-[var(--surface)] hover:border-[var(--accent)] hover:border-opacity-30 transition-all duration-300">
                  {/* Header */}
                  <div className="flex flex-col sm:flex-row sm:items-start sm:justify-between gap-2 mb-4">
                    <div>
                      <div className="flex items-center gap-2 mb-1">
                        <h3 className="font-semibold text-[var(--text)] text-base">{exp.role}</h3>
                        {exp.current && (
                          <span className="mono text-[10px] px-2 py-0.5 rounded-full bg-[var(--accent)] bg-opacity-10 text-[var(--accent)] border border-[var(--accent)] border-opacity-30">
                            current
                          </span>
                        )}
                      </div>
        ...
        {/* Education */}
        <div className="mt-20">
          <p className="mono text-xs text-[var(--accent)] tracking-[0.3em] uppercase mb-8">
            education
          </p>
          <div className="grid md:grid-cols-3 gap-5">
            {[
              ...
              >
                ...
                <p className="text-xs text-[var(--muted)] mb-3">{edu.institution}</p>
                <span className="mono text-xs px-2.5 py-1 rounded-lg bg-[var(--bg)] border border-[var(--border)] text-[var(--accent)]">
                  {edu.grade}
                </span>
              </div>
            ))}
