...
export default function BlogListClient({ posts, activityMap: serverMap }: Props) {
  ...
  // Tag folders + the uncategorised bucket, combined into one list —
  // used to render the accordion rows in the folder view.
  const allFolders = useMemo(() =>
    [...folders, ...(uncategorised.length > 0 ? [{ tag: 'uncategorised', posts: uncategorised }] : [])]
  ...
  return (
    <div>
      ...
      {/* ── FOLDER VIEW — compact accordion rows; each opens its own
             dropdown of posts directly beneath itself ── */}
      {view === 'folders' && (
        <div>
          {allFolders.length === 0 ? (
            ...
            <div className="space-y-2">
              {allFolders.map(({ tag, posts: folderPosts }) => {
                const isOpen = openFolders.has(tag)
                return (
                  <div key={tag}>
                    {/* Folder row — small, click to open the dropdown right below it */}
                    <button
                      onClick={() => toggleFolder(tag)}
                      className={`group flex items-center gap-2.5 w-full px-3.5 py-2.5 border text-left transition-colors duration-150 ${
                        isOpen
                          ? 'border-[var(--accent)] bg-[var(--surface)] rounded-t-lg'
                          : 'border-[var(--border)] bg-[var(--surface)] hover:border-[var(--accent)] rounded-lg'
                      }`}
                    >
                      {isOpen
                        ? <FolderOpen size={15} className="text-[var(--accent)] shrink-0" />
                        : <Folder size={15} className="text-[var(--muted)] group-hover:text-[var(--accent)] transition-colors shrink-0" />
                      }
                      ...
                      <ChevronRight
                        size={14}
                        className={`ml-auto shrink-0 text-[var(--muted)] transition-transform duration-200 ${isOpen ? 'rotate-90 text-[var(--accent)]' : 'group-hover:text-[var(--accent)]'}`}
                      />
                    </button>

                    {/* Dropdown — this folder's posts, attached directly under its row */}
                    {isOpen && (
                      <div className="animate-fade-in border border-t-0 border-[var(--accent)] rounded-b-lg p-3.5 bg-[var(--surface2)]">
